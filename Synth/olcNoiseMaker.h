...
/*
        ...
        From Javidx9 :)
        ~~~~~~~~~~~~~~~
        Hello! Ultimately I don't care what you use this for. It's intended to
   be educational, and perhaps to the oddly minded - a little bit of fun. Please
   hack this, change it and use it in any way you see fit. You acknowledge that
   I am not responsible for anything bad that happens as a result of your
   actions. However this code is protected by GNU GPLv3, see the license in the
        github repo. This means you must attribute me if you use it. You can
   view this license here:
   https://github.com/OneLoneCoder/videos/blob/master/LICENSE Cheers! Author
        ~~~~~~
        Twitter: @javidx9
...
class olcNoiseMaker {
 public:
  olcNoiseMaker(wstring sOutputDevice, unsigned int nSampleRate = 44100,
  ...
  ~olcNoiseMaker() { Destroy(); }

  bool Create(wstring sOutputDevice, unsigned int nSampleRate = 44100,
              unsigned int nChannels = 1, unsigned int nBlocks = 8,
              unsigned int nBlockSamples = 512) {
    m_bReady = false;
    ...
    if (d != devices.end()) {
      ...
      if (waveOutOpen(&m_hwDevice, nDeviceID, &waveFormat,
  ...
  double GetTime() { return m_dGlobalTime; }
    ...
    while (m_bReady) {
      ...
      for (unsigned int n = 0; n < m_nBlockSamples; n++) {
        ...
        if (m_userFunction == nullptr)
          ...
          nNewSample =
              (T)(clip(m_userFunction(m_dGlobalTime), 1.0) * dMaxSample);
